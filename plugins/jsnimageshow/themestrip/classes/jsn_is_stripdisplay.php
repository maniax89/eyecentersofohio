...
class JSNISStripDisplay extends JObject
{
	...
	function standardLayout($args)
	{
		...
		foreach ($images as $image)
		{
			...
			$html .= '<img height=' . $themeData->image_height . ' width=' . $themeData->image_width . ' style="height:' . $themeData->image_height . 'px;  width:' . $themeData->image_width . 'px;" id="themestrip_img_' . $args->random_number . '_' . $i++ . '" src="' . $image->$imageSource . '" border="0" alt="' . $alt . '" title="'.htmlspecialchars($image->title, ENT_QUOTES).'"/>';
			$html .= '</a></li>';
		}
