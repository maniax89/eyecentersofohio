...
class JSNISGridDisplay extends JObject
{
	...
	function standardLayout($args)
	{
        ...
        $objAllows->caption_css 		= $themeData->caption_css;
		$objAllows->show_description	= $themeData->caption_show_description;
        $objAllows->description_css 		= $themeData->description_css;
		...
		$modalID			= 'jsn-'.$this->_themename.'-modal-container-'.$args->random_number;


		if (trim($themeData->caption_css) != '')
		{
			$css .= '#' . $modalID . ' .ppt {';
			$css .= $themeData->caption_css;
			$css .= '}';
		}

		if (trim($themeData->description_css) != '')
		{
			$css .= '#' . $modalID . ' .pp_description {';
			$css .= $themeData->description_css;
			$css .= '}';
		}
		...
		foreach ($images as $image)
		{
			...
			$html .= '<img id="img_'.$args->random_number.'_'.$i++.'" src="' . $image->$imageSource . '" border="0" title="' . htmlspecialchars($image->title, ENT_QUOTES) . '" alt="'.$alt.'"/>';
			$html .= '</a></div>';
		}
		$html .= '</div>';
		...
		$html .= '<script type="text/javascript">
						jsnThemeGridjQuery(function() {
							...
							jsnThemeGridjQuery(window).load(function(){
								...
								jsnThemeGridjQuery(".'.$wrapClass.'").gridtheme('.$themeDataJson.');
								jsnThemeGridjQuery(".'.$wrapClass.'").gridtheme.lightbox({modalID: "' . $modalID . '", rand:"'.$args->random_number.'", allowedData: ' . json_encode($objAllows) . '});
							});
						});
				</script>';
		return $html;
	...
	function getThemeDataStandard($args)
	{
		if (is_object($args))
		{
            ...
            $gridOptions->caption_css		= $themeData->caption_css;
            ...
            $gridOptions->description_css		= $themeData->description_css;
