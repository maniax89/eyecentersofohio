...
//                    "options"    => JSNPbArticleListHelper::getArticleListEasyAuthors(),
    ...
    function element_shortcode($attributes = null, $content = null)
    {
        ...
        if ($arr_params['articlelist_layout'] != JSNPbArticleListHelper::PB_ARTICLE_LIST_LAYOUT) {
            ...
            $cleanTitle = str_replace('$', '&#36;',  $articleFirst['title']);
            $cleanCategoryTitle =  str_replace('$', '&#36;',  $articleFirst['category_title']);
            $html_element .= "<div class='title'><a class='title' href='{$firstUrl}'>{$cleanTitle}</a></div>";
            if (count($firstMetaData) > 1) {
                ...
                if (in_array(JSNPbArticleListHelper::PB_ARTICLE_META_DATA_AUTHOR, $firstMetaData)) {
                    $html_element .= "<div class='created-by'>".JText::sprintf('COM_CONTENT_WRITTEN_BY', $articleFirst['author'])."</div>";
                }
                if (in_array(JSNPbArticleListHelper::PB_ARTICLE_META_DATA_DATE_PUBLISHED, $firstMetaData)) {
                    ...
                    $html_element .= "<div class='published'>".JText::sprintf('COM_CONTENT_PUBLISHED_DATE_ON', JHtml::_('date', $_publishData->format('d M Y'), JText::_('DATE_FORMAT_LC3')))."</div>";
                }
                if (in_array(JSNPbArticleListHelper::PB_ARTICLE_META_DATA_CATEGORY, $firstMetaData)) {
                    ...
                    $html_element .= "<div class='category'> <a href='{$urlCat}'>" . JText::sprintf('COM_CONTENT_CATEGORY', $cleanCategoryTitle)."</a></div>";
                }
                if (in_array(JSNPbArticleListHelper::PB_ARTICLE_META_DATA_HITS, $firstMetaData)) {
                    $html_element .= "<div class='hits'>".JText::sprintf('COM_CONTENT_ARTICLE_HITS', $articleFirst['hits'])."</div>";
                }
                $html_element .= "</div>";
            }
            ...
            if ($attributes['articlelist_show_first_description'] == JSNPagebuilderHelpersType::PB_HELPER_ANSWER_YES) {
				...
				if ($attributes['articlelist_source'] == 'joomla_article')
				{
					
					$articlelistFirstDescription = JSNPbArticleListHelper::wordLimiter($_firstIntroText, (int) $attributes['articlelist_first_description_limit']);	
					$articlelistFirstDescription = JHtml::_('content.prepare', $articlelistFirstDescription);						
				}
				else
				{
					$articlelistFirstDescription = JSNPbArticleListHelper::wordLimiter($_firstIntroText, (int) $attributes['articlelist_first_description_limit']);
				}
                $html_element .= "<div class='articlelist-first-thumbnail'><p>" . $articlelistFirstDescription . "</p></div>";
            }

			if ($arr_params['articlelist_show_read_more_first'] == JSNPagebuilderHelpersType::PB_HELPER_ANSWER_YES) {
                $html_element .= "<div class='articlelist-item-read-more'><a href='{$firstUrl}'>" . JText::_('COM_CONTENT_READ_MORE_TITLE') . "</a></div>";
            }
        ...
        foreach ($articles as $_article) {
            ...
            $cleanTitle = str_replace('$', '&#36;', $_article["title"]);
            if ($arr_params['articlelist_list_style'] == JSNPbArticleListHelper::PB_ARTICLE_LIST_STYLE_THUMBNAIL) {
                ...
                if ($bgImageItemUrl != '')
                {
                	$alt_text = $alt_text ? $alt_text : $cleanTitle;
            ...
            $html_element .= "<div class='title'><a class='title' href='{$url}'>{$cleanTitle}</a></div>";

            if (count($metaData) > 1) {
                $cleanCategoryTitle =  str_replace('$', '&#36;',  $_article['category_title']);
                $html_element .= "<div class='meta-data-wrapper muted'>";
                if (in_array(JSNPbArticleListHelper::PB_ARTICLE_META_DATA_AUTHOR, $metaData)) {
                    $html_element .= "<div class='created-by'>".JText::sprintf('COM_CONTENT_WRITTEN_BY', $_article['author'])."</div>";
                }
                ...
                if (in_array(JSNPbArticleListHelper::PB_ARTICLE_META_DATA_HITS, $metaData)) {
                    $html_element .= "<div class='hits'>".JText::sprintf('COM_CONTENT_ARTICLE_HITS', $_article['hits'])."</div>";
                }
                $html_element .= "</div>";
            }
            ...
            if ($attributes['articlelist_show_description'] == JSNPagebuilderHelpersType::PB_HELPER_ANSWER_YES) {
				...
				if ($attributes['articlelist_source'] == 'joomla_article')
				{
					...
					$articlelistDescription = JSNPbArticleListHelper::wordLimiter($_introText, (int) $attributes['articlelist_description_limit']);
					
					$articlelistDescription = JHtml::_('content.prepare', $articlelistDescription);			
				}
				else
				{
					$articlelistDescription = JSNPbArticleListHelper::wordLimiter($_introText, (int) $attributes['articlelist_description_limit']);
				}
                $html_element .= "<div class='articlelist-item-description'><p>" .$articlelistDescription . "</p></div>";
            }

			if ($arr_params['articlelist_show_read_more'] == JSNPagebuilderHelpersType::PB_HELPER_ANSWER_YES) {
                $html_element .= "<div class='articlelist-item-read-more'><a href='{$url}'>" . JText::_('COM_CONTENT_READ_MORE_TITLE') . "</a></div>";
            }
