...
class plgSystemPagebuilder extends JPlugin
{
	...
	public function onBeforeRender()
	{
		...
		if ( ! preg_match( '/(DocumentHtml|HtmlDocument)/i', get_class($doc) ) ) return;
			...
			if (file_exists(JPATH_ROOT . '/administrator/components/com_pagebuilder/helpers/shortcode.php')) {
				if (class_exists('K2HelperUtilities'))
				{
					...
					if (count($k2Item))
					{
						...
						{
                            $pbdDetect = stripos($doc->getDescription(), '[pb_row');
                            if ($pbdDetect !== false)
                            {
	...
	public function onAfterRender()
	{
		...
		{
			$html = $app->getBody();
			$html = preg_replace("#<script src=\"(.*?)\/jquery.ui.sortable.min.js\"(.*?)></script>#", '',  $html);
			$app->setBody($html);
		}
		...
		$body_content   = str_replace('$', '&dollar;', $body_content);
    ...
    /*
     * This function remove BBCode on og:description tag
     */
    public function onBeforeCompileHead()
    {
		...
		if ($view=='article' && $option=='com_content' && $app->isSite())
        {
            $doc        	= JFactory::getDocument();
            ...
            if (isset($headData['custom']) && !empty($headData['custom']))
            {
                foreach ($headData['custom'] AS &$tag)
                {
                    //Check if exists og:description
                    $ogDescDetect = strpos($tag, 'og:description');
                    if ($ogDescDetect !== false)
                    {
                        //Detect JSN Page builder
                        $pbdDetect = stripos($tag, '[pb_row');
                        if ($pbdDetect !== false)
                        {
                            //Select content form database
                            ...
                            $shortCodeRegex = JSNPagebuilderHelpersShortcode::getShortcodeRegex();
                            $introText = preg_replace(array("#{(.*?)}(.*?){/(.*?)}#s", "/[\n\r]/"), '', $introText);
                            $introText = strip_tags($introText);
                            $introText = JSNPagebuilderHelpersShortcode::removeShortCode($introText, $shortCodeRegex);

                            //Facebook og:description max length = 300
                            $introText = JString::substr($introText, 0 , 300) . "...";

                            //Make change meta tag
                            $tag = "<meta property='og:description' content='".str_replace($arraySearch, $arrayReplace, $introText)."' />";
                        }
                    }
                }
